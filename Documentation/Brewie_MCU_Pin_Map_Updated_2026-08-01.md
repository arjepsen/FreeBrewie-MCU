...
- **`Brewie_MCU_Roadmap_Updated_2026-08-01.md`** = progress, order of work, and remaining steps
- **`Brewie_MCU_Application_Flow_2026-08-01.md`** = top-level program flow and state model
...
## Current update note (2026-08-01)
...
- the MCU reset input is driven from the SOM. Old Brewie Linux mapped this to A13 `PE9`
  through `/dev/brewie-mcu-reset -> /sys/class/gpio/gpio6_pe9`; on the current Olimex
  image the equivalent Linux GPIO is `gpio137`.
...
| 30 | RESET | — | Reset | MCU reset input | Driven from SOM A13 `PE9` / Linux `gpio137`; old alias `/dev/brewie-mcu-reset` | ⚡ |
